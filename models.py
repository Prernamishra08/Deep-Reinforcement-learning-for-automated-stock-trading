...
class DRLEnsembleAgent:
    ...
    def run_ensemble_5_strategy(self,A2C_model_kwargs,PPO_model_kwargs,DDPG_model_kwargs,TD3_model_kwargs,SAC_model_kwargs,timesteps_dict):
        """Ensemble Strategy that combines PPO, A2C and DDPG"""
        print("============Start Ensemble Strategy============")
        # for ensemble model, it's necessary to feed the last state
        # of the previous model to the current model as the initial state
        ...
        for i in range(self.rebalance_window + self.validation_window, len(self.unique_trade_date), self.rebalance_window):
            ...
            print("======SAC Training========")
            model_sac = self.get_model("sac",self.train_env,policy="MlpPolicy",model_kwargs=SAC_model_kwargs)
            model_sac = self.train_model(model_sac, "sac", tb_log_name="sac_{}".format(i), iter_num = i, total_timesteps=timesteps_dict['sac']) #100_000
            ...
            self.DRL_validation(model=model_sac,test_data=validation,test_env=val_env_sac,test_obs=val_obs_sac)
            ...
            if (sharpe_ppo >= sharpe_a2c) & (sharpe_ppo >= sharpe_ddpg) & (sharpe_ppo >= sharpe_td3) & (sharpe_ppo >= sharpe_sac):
                ...
                model_ensemble = self.get_model("a2c",self.train_full_env,policy="MlpPolicy",model_kwargs=A2C_model_kwargs)
                model_ensemble = self.train_model(model_ensemble, "ensemble", tb_log_name="ensemble_{}".format(i), iter_num = i, total_timesteps=timesteps_dict['a2c']) #100_000
            elif (sharpe_ddpg > sharpe_ppo) & (sharpe_ddpg > sharpe_a2c) & (sharpe_ddpg >= sharpe_td3) & (sharpe_ddpg >= sharpe_sac):
                model_use.append('DDPG')

                model_ensemble = self.get_model("ddpg",self.train_full_env,policy="MlpPolicy",model_kwargs=DDPG_model_kwargs)
                model_ensemble = self.train_model(model_ensemble, "ensemble", tb_log_name="ensemble_{}".format(i), iter_num = i, total_timesteps=timesteps_dict['ddpg']) #50_000
            elif (sharpe_td3 > sharpe_ppo) & (sharpe_td3 > sharpe_a2c) & (sharpe_td3 > sharpe_ddpg) & (sharpe_td3 > sharpe_sac):
                model_use.append('TD3')

                model_ensemble = self.get_model("td3",self.train_full_env,policy='MlpPolicy',model_kwargs=TD3_model_kwargs)
                model_ensemble = self.train_model(model_ensemble, "ensemble", tb_log_name="ensemble_{}".format(i), iter_num = i, total_timesteps=timesteps_dict['td3'])
            else:
                model_use.append("SAC")

                model_ensemble = self.get_model("sac",self.train_full_env,policy='MlpPolicy',model_kwargs=SAC_model_kwargs)
                model_ensemble = self.train_model(model_ensemble, "ensemble", tb_log_name="ensemble_{}".format(i), iter_num = i, total_timesteps=timesteps_dict['sac'])
        ...
        df_summary = pd.DataFrame([iteration_list,validation_start_date_list,validation_end_date_list,model_use,a2c_sharpe_list,ppo_sharpe_list,ddpg_sharpe_list, td3_sharpe_list, sac_sharpe_list]).T
        df_summary.columns = ['Iter','Val Start','Val End','Model Used','A2C Sharpe','PPO Sharpe','DDPG Sharpe', 'TD3 Sharpe', 'SAC Sharpe']
